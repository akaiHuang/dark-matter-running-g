...
($\tilde{w}_0 \sim 6 \times 10^{-11}$) at CMB scales,
...
Mistele \emph{et~al.}~\cite{Mistele2024} provide
direct evidence for this prediction: using weak lensing
from DESI Legacy Survey data, they infer circular velocity
curves around isolated galaxies that remain flat out to
$\sim\!1\;\mathrm{Mpc}$---well beyond the expected virial
radii of NFW halos---with no detectable decline.
The NFW profile predicts $v(r) \propto r^{-1/2}$ beyond the
virial radius; the observed indefinitely flat $v(r)$ is
the natural consequence of $\rho \propto r^{-2}$
(equivalently, $G(r)/G_N = 1 + 2k_* r/\pi$ with constant
$k_*$) predicted by the $\tauasym$ framework.
...
$H(z_{\mathrm{rec}}) \approx 2.3 \times 10^{-14}\;
\mathrm{s}^{-1}$ and the first acoustic peak corresponds to
$k_{\mathrm{phys}} \sim 0.01\;\mathrm{Mpc}^{-1}$, giving
\begin{equation}
\tilde{w}_0(z \approx 1100) \sim 6 \times 10^{-11}
\ll 1.
\label{eq:w_dust}
\end{equation}
...
\bibitem{Mistele2024}
T.~Mistele, S.~S. McGaugh, F.~Lelli, J.~Schombert, and P.~Li,
``Indefinitely flat circular velocities and the baryonic
Tully--Fisher relation from weak lensing,''
